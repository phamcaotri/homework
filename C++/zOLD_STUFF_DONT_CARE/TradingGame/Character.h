...
// ------------------------- CONSTRUCTOR -------------------------
        ...
        : Entity(name, coin, buyMultiplier, sellMultiplier) {
            lv = Level();
        }
...
// ------------------------- METHODS -------------------------
        ...
        void BuyFrom(Entity &seller, int seller_item_index, int amount = 1) override {
            Item item = seller.getInventory().getItem(seller_item_index);
            float price = round(item.getPrice()*seller.getBuyMultiplier())*amount;
            float exp = round(price*EXP_MULTIPLIER);
            if (getCoin() >= price) {
                addCoin(-price);
                seller.addCoin(price);
                seller.getInventory().removeItem(seller_item_index, amount);
                getInventory().addItem(item, amount);
                addExp(exp);
        ...
        void SellTo(Entity &buyer, int seller_item_index, int amount = 1) override {
            ...
            if (buyer.getCoin() >= price) {
                addCoin(price);
                buyer.addCoin(-price);
                buyer.getInventory().addItem(item, amount);
                getInventory().removeItem(seller_item_index, amount);
