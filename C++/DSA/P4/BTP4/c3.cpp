...
DNode* findFirstNodeEqual(DList &l, int x) {
    DNode *this_node = l.pHead;
    while (this_node != NULL) {
        if (this_node->info == x) {
            return this_node;
...
void addAfter(DList &l, int x, int y) {
    DNode* this_node = findFirstNodeEqual(l, x);
    if (this_node == NULL) {
    ...
    if (l.pHead == NULL) {
        ...
        new_node->pNext = this_node->pNext;
        new_node->pPrev = this_node;
